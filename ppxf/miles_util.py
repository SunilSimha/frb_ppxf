...
class miles(object):
    ...
    def __init__(self, pathname, velscale, FWHM_gal, wave_gal=None,
        ...
        :param wave_gal: optional wavelength of galaxy spectrum in Angstrom.
            Required if giving FWHM_gal as vector
        ...
        if not isinstance(FWHM_gal, float):
            if wave_gal is None:
                raise ValueError('If FWHM_gal is given per pixel, corresponding '
                                 'wavelength array (wave_gal) must be provided.')
        ...
        ssp = hdu[0].data
        h2 = hdu[0].header
        lam_temp = np.arange(h2['CRVAL1'],h2['CRVAL1']+h2['CDELT1']*(h2['NAXIS1']),
                             h2['CDELT1'])
        ...
        for j, age in enumerate(ages):
            for k, metal in enumerate(metals):
                ...
                if np.isscalar(FWHM_gal):
                    ...
                    try:
                        interp = interp1d(wave_gal, FWHM_gal,fill_value='extrapolate')
                        sig_interp = interp(lam_temp)
                        ssp = util.gaussian_filter1d(ssp, sig_interp)  # convolution with variable sigma
                    except:
                        import pdb; pdb.set_trace()
        ...
        self.normfactor = 1./np.median(templates)
        self.templates = templates * self.normfactor  # Normalize by a scalar
...
#       MC, Garching, 11 January 2016
    ...
    def mass_to_light(self, weights, band="r", quiet=False):
        ...
        slope1, MH1, Age1, m_no_gas = readcol(file1, usecols=[1, 2, 3, 4])
        ...
        print('Mass: {}'.format(np.sum(weights*mass_no_gas_grid)))
        print('Lum: {}'.format(np.sum(weights*lum_grid)))
...
###############################################################################
# MODIFICATION HISTORY:
#   V1.0.0: Adapted from mass_to_light by J. Burchett 22 November 2017


    def total_mass(self, weights, quiet=False):
        """
        Computes the total mass of living stars and stellar remnants
        in models fitted, given the weights producedin output by pPXF.
        A Salpeter IMF is assumed (slope=1.3).
