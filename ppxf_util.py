...
def log_rebin(lamRange, spec, oversample=1, velscale=None, flux=False):
    ...
    if velscale is None:                     # Velocity scale is set by user
        velscale = np.diff(logLim)[0]/m*c    # Only for output; Needs to be a scalar
    else:
        logScale = velscale/c
...
def gaussian(logLam_temp, line_wave, FWHM_gal, pixel=True):
    ...
    to machine precision regardless of sigma.
...
def emission_lines(logLam_temp, lamRange_gal, FWHM_gal, pixel=True,
    ...
    :param limit_doublets: Set this to True to limit the rato of the
      ...
      - EXAMPLE: For the [SII] doublet, the adopted ratios for the templates are
            ratio_d1 = flux([SII]6716)/flux([SII]6731) = 0.44
            ratio_d2 = flux([SII]6716)/flux([SII]6731) = 1.43.
        ...
        the total flux and true lines ratio of the [SII] doublet are
            ...
            ratio_total[SII] = (0.44*flux_1 + 1.43*flux_2)/flux_total.
      - Similarly, for [OII], the adopted ratios for the templates are
            ratio_d1 = flux([OII]3729)/flux([OII]3726) = 0.28
            ratio_d2 = flux([OII]3729)/flux([OII]3726) = 1.47.
        If pPXF prints (and returns in pp.gas_flux)
            flux([OII]3726_d1) = flux_1
            flux([OII]3726_d2) = flux_2
        the total flux and true lines ratio of the [OII] doublet are
            flux_total = flux_1 + flux_2
            ratio_total[OII] = (0.28*flux_1 + 1.47*flux_2)/flux_total.
    ...
    if tie_balmer:
        ...
        # Balmer:  H7     H6        H5      Heps    Hdelta   Hgamma    Hbeta   Halpha
...
#
# MODIFICATION HISTORY:
#	V1.0.0: Started new function based on EMISSION_LINES to allow for masked out
#     spectral regions

def emission_lines_mask(logLam_temp, maskedLam_gal, FWHM_gal, pixel=True,
				   tie_balmer=False, limit_doublets=False, vacuum=False):
	"""
	Generates an array of Gaussian emission lines to be used as gas templates in PPXF.
	...
	:param logLam_temp: is the natural log of the wavelength of the templates in
		Angstrom. logLam_temp should be the same as that of the stellar templates.
	:param maskedLam_gal: is the estimated rest-frame fitted wavelength range
		Whereas in the `emission_lines' function where user specifies a wavelength
		range, the user can specify a wavelength array with gaps where data are not
		to be fitted.  This routine finds these gaps and does not include emission 
		lines within them.
	:param FWHM_gal: is the instrumantal FWHM of the galaxy spectrum under study
		in Angstrom. One can pass either a scalar or the name "func" of a function
		func(wave) which returns the FWHM for a given vector of input wavelengths.
	...
	if tie_balmer:
		...
		if vacuum:
			wave = air_to_vac(wave)
		gauss = gaussian(logLam_temp, wave, FWHM_gal, pixel)
		todot_default = [0.0530, 0.0731, 0.105, 0.159, 0.259, 0.468, 1., 2.86]
		emission_lines = gauss.dot(todot_default)
		line_names = ['Balmer']
		line_wave = np.mean(wave)
	...
	# This is important to avoid instabilities in the pPXF system solution
	#
	### First, identify max and min wavelength range 
	crits = [(line_wave > maskedLam_gal[0]),(line_wave < maskedLam_gal[-1])]
	### Check for big gaps in the wavelengths provided
	wavediff = maskedLam_gal[1:]-maskedLam_gal[:-1]
	sddiff = np.std(wavediff)
	bigdiffs = np.where(wavediff>(np.median(wavediff)+10.*sddiff))[0]
	for i,idx in enumerate(bigdiffs):
		crits.append((line_wave < maskedLam_gal[idx]) | 
					(line_wave > maskedLam_gal[idx+1]))
	critmask = np.all(crits,axis=0)
	emission_lines = emission_lines[:, critmask]

	line_names = line_names[critmask]
	if isinstance(line_names,str): 
		line_names = [line_names]
	line_wave = line_wave[critmask]
	
	#for i in range(np.shape(emission_lines)[1]):
	#	smallvals = np.where(np.abs(emission_lines[:,i])<1e-5)[0]
	#	emission_lines[:,i][smallvals]=0
